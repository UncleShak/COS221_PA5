...
// Dynamic agency dashboard — uses $agency, $stats, $packages, $recentBookings, $flash
// All user output is escaped with htmlspecialchars() via the hsc() helper.
...
<div class="sg-page" style="min-height: 100vh; padding: 120px 2rem 4rem; background: transparent;">
  <div style="max-width: 1200px; margin: 0 auto; width: 100%;">
    ...
    <!-- ── Flash Message ───────────────────────────────────────── -->
    <?php if ($flash): ?>
      <div class="toast <?= $flash['type'] === 'success' ? 't-success' : '' ?>"
           style="margin-bottom: 1.5rem; max-width: 100%;">
        <div>
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:3rem; flex-wrap:wrap; gap:1.5rem;">
      ...
      <a href="/agency/package/create" class="btn-primary"
         style="border-radius:var(--r-pill); padding:0.8rem 2rem; text-decoration:none;">
        + New Package
      </a>
    </div>
    ...
    <div class="stat-row" style="display:grid; grid-template-columns:repeat(4,1fr); gap:1.5rem; margin-bottom:3rem;">
      ...
      <div class="glass-heavy" style="border-radius:var(--r-xl); padding:1.8rem;">
        <div style="font-size:0.8rem; font-weight:600; color:var(--text-muted); text-transform:uppercase; letter-spacing:.08em; margin-bottom:.6rem;">
          Total Packages
        </div>
        <div style="font-size:2.4rem; font-weight:700; color:var(--ocean); line-height:1;">
          <?= (int) ($stats['total_packages'] ?? 0) ?>
        </div>
        <div style="font-size:0.82rem; color:var(--text-muted); margin-top:.4rem;">
          <?= (int) ($stats['published'] ?? 0) ?> active · <?= (int) ($stats['drafts'] ?? 0) ?> drafts
        </div>
      </div>
      ...
      <div class="glass-heavy" style="border-radius:var(--r-xl); padding:1.8rem;">
        ...
        <div style="font-size:2.4rem; font-weight:700; color:var(--teal); line-height:1;">
      ...
      <div class="glass-heavy" style="border-radius:var(--r-xl); padding:1.8rem;">
        ...
        <div style="font-size:2.4rem; font-weight:700; color:var(--deep-blue); line-height:1;">
          R <?= number_format((float) ($stats['total_revenue'] ?? 0), 0) ?>
        </div>
      ...
      <div class="glass-heavy" style="border-radius:var(--r-xl); padding:1.8rem;">
        ...
        <div style="font-size:2.4rem; font-weight:700; color:var(--warning); line-height:1;">
          <?= $stats['avg_rating'] ? hsc($stats['avg_rating']) : '–' ?>
        </div>
        <div style="font-size:0.82rem; color:var(--text-muted); margin-top:.4rem;">
          from <?= (int) ($stats['review_count'] ?? 0) ?> review<?= $stats['review_count'] != 1 ? 's' : '' ?>
        </div>
      </div>

    </div>

    <!-- ── Main Grid: Package Table + Activity ─────────────────── -->
    <div style="display:grid; grid-template-columns:2fr 1fr; gap:2rem; align-items:start;">
      ...
      <div class="glass-clear" style="border-radius:var(--r-xl); overflow:hidden;">
        ...
        <?php if (empty($packages)): ?>
          <div style="padding:3rem; text-align:center; color:var(--text-muted);">
            No packages yet.
            <a href="/agency/package/create" style="color:var(--ocean); font-weight:600;">Create your first →</a>
          </div>
        <?php else: ?>
            ...
            <table style="width:100%; border-collapse:collapse;">
              <thead>
                <tr style="background:rgba(255,255,255,0.3);">
                  <th style="padding:.9rem 1.2rem; text-align:left; font-size:.8rem; font-weight:600; color:var(--text-muted); text-transform:uppercase; letter-spacing:.06em; white-space:nowrap;">Title</th>
                  ...
                  <th style="padding:.9rem 1.2rem; text-align:left; font-size:.8rem; font-weight:600; color:var(--text-muted); text-transform:uppercase; letter-spacing:.06em;">Rating</th>
              ...
              <tbody>
                <?php foreach ($packages as $i => $pkg): ?>
                  <?php
                    // FIX: status ENUM is 'active' not 'published'
                    ...
                    // Human-readable label for 'active'
                    $statusLabel = match ($pkg['status']) {
                        'active'   => 'Active',
                        'draft'    => 'Draft',
                        'archived' => 'Archived',
                        default    => ucfirst(hsc($pkg['status'])),
                    };
                  ?>
                  <tr style="border-top:1px solid rgba(255,255,255,0.3); <?= $i % 2 ? 'background:rgba(255,255,255,0.08)' : '' ?>">
                    ...
                    <td style="padding:.9rem 1.2rem;">
                      ...
                      <div style="font-size:.78rem; color:var(--text-muted); margin-top:.15rem;">
                        <?= (int) $pkg['duration_days'] ?> days
                        <?php if ($pkg['is_group_trip']): ?>
                          &nbsp;<span class="badge badge-blue">Group Trip</span>
                        <?php endif; ?>
                      </div>
                    </td>

                    <td style="padding:.9rem 1.2rem; white-space:nowrap;">
                      R <?= number_format((float) $pkg['base_price'], 0) ?>
                      <span style="font-size:.78rem; color:var(--text-muted);">pp</span>
                    </td>
                    ...
                    <td style="padding:.9rem 1.2rem; font-size:.95rem;">
                      <?= (int) $pkg['booking_count'] ?>
                    </td>

                    <td style="padding:.9rem 1.2rem; font-size:.95rem;">
                      <?= $pkg['average_rating'] ? '⭐ ' . hsc($pkg['average_rating']) : '—' ?>
                    </td>

                    <td style="padding:.9rem 1.2rem; text-align:center; white-space:nowrap;">
                      <a href="/agency/package/edit?id=<?= (int) $pkg['package_id'] ?>"
                         class="btn-secondary"
                         style="padding:.35rem .9rem; border-radius:var(--r-pill); font-size:.82rem; text-decoration:none; margin-right:.4rem;">
                        Edit
                      </a>

                      <?php if ($pkg['status'] !== 'archived'): ?>
                        <form method="POST" action="/agency/package/archive"
                              style="display:inline;"
                              onsubmit="return confirm('Archive this package?')">
                          <input type="hidden" name="package_id" value="<?= (int) $pkg['package_id'] ?>">
                          <button type="submit"
                                  style="background:none; border:1px solid var(--danger); color:var(--danger); border-radius:var(--r-pill); padding:.35rem .9rem; font-size:.82rem; cursor:pointer;">
                            Archive
                          </button>
                        </form>
                      <?php endif; ?>
      ...
      <div class="glass-clear" style="border-radius:var(--r-xl); overflow:hidden;">
        ...
        <?php if (empty($recentBookings)): ?>
          <div style="padding:2rem; text-align:center; color:var(--text-muted); font-size:.9rem;">
            No bookings yet.
          </div>
        <?php else: ?>
          <div style="padding:.5rem 0;">
            <?php foreach ($recentBookings as $booking): ?>
              <?php
                $bStatusColor = match ($booking['status']) {
              ...
              <div style="padding:1rem 1.5rem; border-bottom:1px solid rgba(255,255,255,0.2);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:.5rem;">
                  ...
                  <span style="font-size:.75rem; font-weight:600; color:<?= $bStatusColor ?>; white-space:nowrap;">
                    <?= ucfirst(hsc($booking['status'])) ?>
                  </span>
                </div>
                <div style="display:flex; justify-content:space-between; margin-top:.5rem;">
                  <!-- booking_date is the alias set in getRecentBookings() for b.created_at -->
                  <span style="font-size:.78rem; color:var(--text-muted);"><?= hsc(date('d M Y', strtotime($booking['booking_date']))) ?></span>
                  <span style="font-size:.82rem; font-weight:600; color:var(--ocean);">R <?= number_format((float) $booking['total_price'], 0) ?></span>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
